...
RSpec.shared_examples :transactional_messages_periodic_job do
  let(:created_at) do
    1.second.ago - Ohm::TransactionalMessagesJob::MAX_DURATION
  end
...
RSpec.describe Ohm::TransactionalMessagesPeriodicJob do
  describe "#perform" do
    context "inbox" do
      let(:periodic_job) { Ohm::Inbox::MessagesPeriodicJob.new }
      let(:box) { Dummy::Inbox }
    ...
    context "outbox" do
      let(:periodic_job) { Ohm::Outbox::MessagesPeriodicJob.new }
